	...
	<ul id="user-${type}" class="fixed-height">
	    <c:if test="${type == t['metas.answers_lowercase']}">
			<c:forEach var="answer" items="${list}">
					<li class="ellipsis advanced-data-line"><span class="counter">${fa:toFa(answer.voteCount)}</span> <tags:questionLinkFor answer="${answer}"/></li>
			</c:forEach>
		</c:if>
		<c:if test="${type != t['metas.answers_lowercase']}">
			<c:forEach var="question" items="${list}">
					<li class="ellipsis advanced-data-line"><span class="counter">${fa:toFa(question.voteCount)}</span> <tags:questionLinkFor question="${question}"/></li>
			</c:forEach>
		</c:if>
	</ul>
